import torch
from lib.fluid import CellType
...
def velocityVorticity(U, flags):
    ...
    if (not is_3d):
        ...
        Uijk_p = Uijk.clone()
        Uijk_p[:,0] = U.narrow(x, 1, w-2).narrow(y, 2, h-2).select(1,0)
        Uijk_p[:,1] = U.narrow(x, 2, w-2).narrow(y, 1, h-2).select(1,1)
    else:
    ...
    #zeros_border_x = torch.zeros_like(vorticity[:,:,:,0,1:-1])
    #zeros_border_y = torch.zeros_like(vorticity[:,:,:,1:-1,0])
    
    #vorticity[:,:,:,-2,1:-1]= zeros_border_x
    #vorticity[:,:,:,1:-1,-2]= zeros_border_y

    vorticity[:,:,:,-2,1:-1]= vorticity[:,:,:,-3,1:-1]
    vorticity[:,:,:,1:-1,-2]= vorticity[:,:,:,1:-1,-3]
