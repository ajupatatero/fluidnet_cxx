...
initial_filename = glob.os.path.join(conf['dataDir'], conf['dataset'], 'tr', '000036', '000036_pyTen.pt')
...
try:
    ...
    with torch.no_grad():
        ...
        while (it < max_iter):
            folder_1 = '{0:06}'.format(4)
            folder_2 = '{0:06}'.format(it*4)
            initial_filename = glob.os.path.join(conf['dataDir'], conf['dataset'], 'tr', folder_1, folder_2+'_pyTen.pt')
