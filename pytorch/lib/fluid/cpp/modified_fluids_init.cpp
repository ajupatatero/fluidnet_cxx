   ...
   const int batch = 64
    ...
    for( int i = 0; i < w; i = i + 1 ) {
        for( int j = 0; j < h; j = j + 1 ) {
            T Intermediate = flags[batch][0][i][j][0];
            int* flag_val = Intermediate.data<int>();
            bool isfluid = (flag_val > 0);
            //If the corresponding position is fluid: Algo from Bridson pg 65
            if (isfluid) {
               Temporal[batch][0][i][j][0] = residual[batch][0][i][j][0] - (A_next_i[batch][0][i-1][j][0]*Precon[batch][0][i-1][j][0]*Q[batch][0][i-1][j][0])
                - (A_next_j[batch][0][i][j-1][0]*Precon[batch][0][i][j-1][0]*Q[batch][0][i][j-1][0]);
            }
            
            // We have just solved the equation Lq =r
            Q[batch][0][i][j][0] = Temporal[batch][0][i][j][0]*Precon[batch][0][i][j][0];
    ...
    for( int i = w; i > 0; i = i - 1 ) {
        for( int j = h; j > 0; j = j - 1 ) {
            ...
            //If the corresponding position is fluid: Algo from Bridson pg 65
            if (isfluid) {
                Temporal[batch][0][i][j][0] = Q[batch][0][i][j][0] - (A_next_i[batch][0][i][j][0]*Precon[batch][0][i][j][0]*Z[batch][0][i+1][j][0])
                - (A_next_j[batch][0][i][j][0]*Precon[batch][0][i][j][0]*Z[batch][0][i][j-1][0]);
            }
            
            // We have just solved the equation Lq =r
            Z[batch][0][i][j][0] = Temporal[batch][0][i][j][0]*Precon[batch][0][i][j][0];
        ...
        for( int batch=0; batch<bsz; batch = batch +1){
            ...
            for( int i = 0; i < w; i = i + 1 ) {
                for( int j = 0; j < h; j = j + 1 ) {
                   ...
                   if (isfluid) {
                       ...
                       E_diag[batch][0][i][j][0] = A_diag[batch][0][i][j][0] - (A_next_i[batch][0][i-1][j][0]*Precon[batch][0][i-1][j][0]).pow(2) - (A_next_j[batch][0][i][j-1][0]*
                        Precon[batch][0][i][j][0]).pow(2) - tau * (A_next_i[batch][0][i-1][j][0]*(A_next_j[batch][0][i-1][j][0]*(Precon[batch][0][i-1][j][0]).pow(2))+ 
                        A_next_j[batch][0][i][j-1][0]*A_next_i[batch][0][i][j-1][0]*(Precon[batch][0][i][j-1][0]).pow(2));
                   }
                
                   // Chek if the value es too low (sec factor * diag). If so, E_Diag = A_diag
                   T Inter_1 = E_diag[batch][0][i][j][0]-Safety * A_diag[batch][0][i][j][0];
                   float* Diag_value = Inter_1.data<float>();
                   bool isSmall = (Diag_value < 0);
                   if ( isSmall) {
                       E_diag[batch][0][i][j][0] = A_diag[batch][0][i][j][0];
                   }
                
                   //Finally, the precondtionner will be equal to 1/sqrt(E)
                   Precon[batch][0][i][j][0]= 1 / ((E_diag[batch][0][i][j][0]).pow(0.5));
                }
            }
                
            // Once we have calculated the preconditioner, we should now solve z = Mr
            T z = Precon_Z(flags, div, residual, Precon, A_next_i,A_next_j, batch);
            //T Sigma = std::inner_product(z.argmin(), z.argmax(), residual.argmin(), 0);
            float Sigma =10;
            T s = residual;
            float alpha = 0;
            ...
            for( int i = 0; i < w; i = i + 1 ) {
               for( int j = 0; j < h; j = j + 1 ) {
                ...
                S_minus_j[batch][0][i][j][0] = s[batch][0][i][j-1][0];
                S_plus_j[batch][0][i][j][0] = s[batch][0][i][j+1][0];                

                S_minus_i[batch][0][i][j][0] = s[batch][0][i-1][j][0];
                S_plus_i[batch][0][i][j][0] = s[batch][0][i+1][j][0];
                }
            }
            ...
            while (true) {
                ...
                z = Precon_Z(flags, div, residual, Precon, A_next_i,A_next_j, batch);
                float sigma_new =10;
                //T sigma_new = std::inner_product(z.begin(), z.end(), residual.begin(), 0);
