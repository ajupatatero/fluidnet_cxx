...
try:
    ...
    with torch.no_grad():
        ...
        # We declare the beginning, Chord of the profile and thicknes
        alpha_deg = simConf['alpha_deg']
        alpha= alpha_deg*2*np.pi/360

        Chord = np.float(resX/20)
        centerX = np.int(resX/2)-np.int(np.sin(alpha)*Chord/2)
        centerY = np.int(resX/2)-np.int(np.cos(alpha)*Chord/2)
        thick = 0.12
        alpha_deg = simConf['alpha_deg']

        alpha= alpha_deg*2*np.pi/360

        print( "First X ", centerX, " Y ", centerY, " Chord ", Chord, " Thicknes ", thick)
        ...
        #Only foer the VK and the NACA0012  
        ...
        if batch_dict['Test_case']== 'VK': 
           ...
           """
           Creates a NACA profile in the flags. It will start in the point centerX and centerY. 
           """
           ...
           # Create translated Coordinates. Note that coordinates are changed for coherence with
           # the analytic formula, and normalized by the chrod lenght!

           X_shifted = ((Y - centerY).float()/Chord)*np.cos(alpha)+((X - centerX).float()/Chord)*np.sin(alpha)
           Y_shifted = abs(-((Y - centerY).float()/Chord)*np.sin(alpha)+((X - centerX).float()/Chord)*np.cos(alpha))


           dist_from_ax = 5*thick*(0.2969*(X_shifted)**(0.5)-0.126*(X_shifted)-0.3516*(X_shifted)**2\
                   +0.2843*(X_shifted)**3-0.1015*(X_shifted)**4)

           print(" dist from ax ", dist_from_ax[0,2300:2500,1100:1300])
           print(" dist from ax ", dist_from_ax[0,2300:2500,1100:1300])

           #mask_cylinder = (X_shifted > 0) 
           mask_cylinder = (Y_shifted <= dist_from_ax) & (X_shifted > 0) & (X_shifted < 1)
        ...
        while (it < max_iter):
            ...
            Chord_int = np.int(Chord)
            print("Center Y ", centerY+3*Chord_int," Center X ", centerX)
            Probe_U_y[it]=batch_dict['U'][0,0,0,np.int(resX/2)+10*Chord_int,np.int(resX/2)].cpu().data.numpy()
            Probe_U_x[it]=batch_dict['U'][0,1,0,np.int(resX/2)+10*Chord_int,np.int(resX/2)].cpu().data.numpy()
