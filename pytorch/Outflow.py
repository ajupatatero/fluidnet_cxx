...
try:
    ...
    with torch.no_grad():
        ...
        method = arguments.simMethodSelect or simConf['simMethod']
        #it = 0
        ...
        plume_scale = arguments.jetVelocity or simConf['injectionVelocity']
        ...
        while (it < max_iter):
            ...
            if (it% outIter == 0):
                ...
                tensor_vel_not_centered = batch_dict['U'].clone()
                img_velx_not_centered = torch.squeeze(tensor_vel_not_centered[:,0]).cpu().data.numpy()
                img_vely_not_centered = torch.squeeze(tensor_vel_not_centered[:,1]).cpu().data.numpy()
                ...
                np.save(filename4,img_vely_not_centered[minY:maxY,minX:maxX])

                filename4a = folder + '/Ux_output_{0:05}'.format(it)
                np.save(filename4a,img_velx_not_centered[minY:maxY,minX:maxX])
                ...
                filename11 = folder + '/Time_Pressure'
